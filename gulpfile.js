...
gulp.task('styles', () => {
  return gulp
    ...
    .pipe(sass().on('error', sass.logError)) // преобразование в sass, если преобразование не получилось то мы получим ошибку
    .pipe(concat('finishedCSS.css')) // конкатенация файлов
    .pipe(
      autoPrefixer({
        cascade: false,
      })
...
gulp.task('exportWebP', () => {
  return gulp
    ...
    .pipe(gulp.dest('./build/compressIMG/'));
});
// /*  **-любые файлы */
